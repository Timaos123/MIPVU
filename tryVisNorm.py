...
def visNormal(myArr,deleteOutlier=True,lineColor="k",histColor="#696969",nameOfData="original data",partNum=5):
    ...
    myArrCoor=np.array([[(realMin+i*partItemScale+realMin+(i+1)*partItemScale)/2,myArr[np.logical_and.reduce([myArr>=realMin+i*partItemScale,myArr<realMin+(i+1)*partItemScale])].size/myArrTotalSize] for i in range(partNum)])#get the distribution of the coordinate data 获取原数据柱状分布
    pltBar=plt.bar(x=myArrCoor[:,0],height=myArrCoor[:,1],width=(realMax-realMin)/partNum,color=histColor,edgecolor='w')#plot 输出柱状图
    ...
    normSize=500000
    normArr=np.random.normal(loc=myMean,scale=myStd,size=normSize)#get the data of normal distribution (ND)获取正态分布随机样本
    ...
    ndCoor=np.array([[(ndMin+i*ndPartScale+ndMin+(i+1)*ndPartScale)/2,normArr[np.logical_and.reduce([normArr>=ndMin+i*ndPartScale,normArr<ndMin+(i+1)*ndPartScale])].size/normSize] for i in range(partNum)])#get the coordinate of ND 获取随机样本坐标
